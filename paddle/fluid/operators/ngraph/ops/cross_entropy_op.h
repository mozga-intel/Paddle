...
void BuildCrossEntropyNode(
  ...
  auto dummy_out = paddle::platform::GetOutputNode(op, "Y", ngb_node_map);
  if (dummy_out && dummy_out->get_shape() != xe->get_shape()) {
    xe = paddle::platform::NgReshaper(xe, dummy_out->get_shape());
  }
...
void BuildCrossEntropyGradNode(
  ...
  if (x_shape.size() - dy_shape.size() > 1) {
    ngraph::Shape dy_reshape_shape;
    for (int i = 0; i < x_shape.size() - 1; ++i) {
      dy_reshape_shape.push_back(x_shape.at(i));
    }
    dy_reshape = paddle::platform::NgReshaper(dy_reshape, dy_reshape_shape);
  }
